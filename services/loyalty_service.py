...
class LoyaltyService:
    ...
    async def validate_coupon(
        ...
        Returns:
            CouponValidationResult with validation details
        """
        try:
            # Get coupon
            ...
            if coupon.status != CouponStatus.ACTIVE:
                return CouponValidationResult(
                    is_valid=False,
                    coupon=coupon,
                    error_message=f"Coupon is {coupon.status.value}"
                )
    ...
    async def apply_coupon(
        ...
        try:
            ...
            redemption = await self.create_coupon_redemption(tenant_id, redemption_data)
            ...
            # If usage limit reached, mark as used
            if coupon.usage_limit and coupon.usage_count + 1 >= coupon.usage_limit:
                update_data.status = CouponStatus.USED
    ...
    async def create_coupon_redemption(
        ...
        Args:
            tenant_id: Tenant identifier
            redemption_data: Redemption data
            
        Returns:
            Created CouponRedemptionResponse
        """
        try:
            # Prepare data for insertion
            ...
            if not result.data:
                raise ValueError("Failed to create coupon redemption")
            
            return CouponRedemptionResponse(**result.data[0])
            
        except Exception as e:
            logger.error(f"Error creating coupon redemption: {e}")
            raise
    ...
    async def process_birthday_coupons(
        ...
        Returns:
            List of DistributionLogResponse objects
        """
        try:
            # Get birthday distribution rules
            result = self.db.table("automated_distribution_rules").select("*").eq(
                "tenant_id", tenant_id
            ...
            for rule_data in result.data:
                rule = AutomatedDistributionRuleResponse(**rule_data)
                
                # Get customers with birthdays today
                # This would query customer profiles with birthday field
                # For now, we'll simulate with a placeholder
                # TODO: Implement actual birthday checking from customer profiles
                
                # Simulate finding customers with birthdays
                birthday_customers = await self._get_customers_with_birthday_today(tenant_id, today)
                
                for customer_phone in birthday_customers:
                    try:
                        # Create birthday coupon
                        coupon_data = CouponCreate(
                            coupon_code=f"BDAY{today.strftime('%Y%m%d')}{customer_phone[-4:]}",
                            coupon_type=CouponType.BIRTHDAY,
                            description=f"Happy Birthday! {rule.description or 'Special birthday discount'}",
                        ...
                        log_data = DistributionLogCreate(
                            ...
                            customer_phone=customer_phone,
                            coupon_id=coupon.id,
                            distribution_type=rule.rule_type,
                            trigger_data={"birthday_date": today.isoformat()},
                            status="success"
                        )
                        
                        log = await self.create_distribution_log(tenant_id, log_data)
                        logs.append(log)
                        
                        # Update rule statistics
                        await self._update_rule_statistics(tenant_id, rule.id)
                        
                    except Exception as e:
                        logger.error(f"Error processing birthday coupon for {customer_phone}: {e}")
    ...
    async def process_anniversary_coupons(
        ...
        """
        try:
            # Get anniversary distribution rules
            result = self.db.table("automated_distribution_rules").select("*").eq(
                "tenant_id", tenant_id
            ...
            for rule_data in result.data:
                ...
                # Get customers with purchase anniversaries
                # This would query purchase history for customers with first purchase anniversary
                # For now, we'll simulate with a placeholder
                # TODO: Implement actual anniversary checking from purchase history
                
                anniversary_customers = await self._get_customers_with_anniversary_today(tenant_id, today)
                
                for customer_phone in anniversary_customers:
                    try:
                        # Create anniversary coupon
                        coupon_data = CouponCreate(
                            coupon_code=f"ANNIV{today.strftime('%Y%m%d')}{customer_phone[-4:]}",
                            coupon_type=CouponType.ANNIVERSARY,
                            description=f"Thank you for being a loyal customer! {rule.description or 'Special anniversary discount'}",
                            discount_type=rule.discount_type,
                            discount_value=rule.discount_value,
                            min_purchase_amount=rule.trigger_conditions.get("min_purchase_amount"),
                            max_discount_amount=rule.trigger_conditions.get("max_discount_amount"),
                        ...
                        logger.error(f"Error processing anniversary coupon for {customer_phone}: {e}")
    ...
    async def create_distribution_log(
        self,
        tenant_id: str,
        log_data: DistributionLogCreate
    ) -> DistributionLogResponse:
        """
        Create a distribution log record
        ...
        Returns:
            Created DistributionLogResponse
        """
        try:
            # Prepare data for insertion
            insert_data = log_data.dict()
            insert_data["tenant_id"] = tenant_id
            insert_data["distributed_at"] = datetime.now().isoformat()
            
            # Insert into database
            result = self.db.table("distribution_logs").insert(insert_data).execute()
            
            if not result.data:
                raise ValueError("Failed to create distribution log")
            
            return DistributionLogResponse(**result.data[0])
            
        except Exception as e:
            logger.error(f"Error creating distribution log: {e}")
            raise
    ...
    async def get_automated_distribution_summary(
        ...
        Returns:
            AutomatedDistributionSummary object
        """
        try:
            # Get total rules
            ...
            total_rules = len(rules_result.data) if rules_result.data else 0
            ...
            active_rules = len(active_rules_result.data) if active_rules_result.data else 0
            ...
            total_distributions = len(logs_result.data) if logs_result.data else 0
            ...
            # Get coupons generated by automated rules
            coupons_result = self.db.table("coupons").select("*").eq(
                "tenant_id", tenant_id
            ).in_("coupon_type", ["birthday", "anniversary", "welcome", "loyalty"]).execute()
            
            total_coupons_generated = len(coupons_result.data) if coupons_result.data else 0
            
            # Calculate average discount value
            average_discount_value = 0.0
            if coupons_result.data:
                total_discount = sum(c.get("discount_value", 0) for c in coupons_result.data)
                average_discount_value = total_discount / total_coupons_generated
            
            # Get top rules by distribution count
            top_rules = []
            if logs_result.data:
                # Group by rule_id
                rule_counts = {}
            ...
            # Get distribution trends (last 30 days)
            thirty_days_ago = (datetime.now() - timedelta(days=30)).isoformat()
            trends_result = self.db.table("distribution_logs").select(
                "date(distributed_at)", "count"
            ).eq("tenant_id", tenant_id).gte(
                "distributed_at", thirty_days_ago
            ).group("date(distributed_at)").order(
                "date(distributed_at)", desc=True
            ).execute()
    ...
    async def _get_customers_with_birthday_today(
        ...
        """
        Get customers with birthdays today (placeholder implementation)
    ...
    async def _get_customers_with_anniversary_today(
        ...
        """
        Get customers with purchase anniversaries today (placeholder implementation)
